...
caption_4 <- 'Variance explained and cumulative variance mapped across PCs'
caption_5 <- 'Biplot of first two principal components colorized by subtype'
caption_8 <- 'Normalized intensity values for significantly differentiated probes in gene expression analysis'
```
...
bar_cols <-c('Variance Explained'='light blue')
  variance_tibble %>%
    #corrects the order of PCs on x-axis
    mutate(principal_components = fct_reorder(principal_components, -variance_explained)) %>%
    ggplot() + 
